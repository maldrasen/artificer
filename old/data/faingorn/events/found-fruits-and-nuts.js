Event.build('found-fruits-and-nuts', {
  ...
  stages:[{
    pages:[
      ...
      { playerSpeaker:true, text:`I give {{him}} a good looking over as {{he}} grows near. If possible {{he}}'s even
          more filthy than {{he}} was this morning, covered in dirt with berry juice staining {{his}} arms. "I've been
          thinking though. What you're doing right now is inefficient."` },
    ...
    pages:[
      { text:`I shake my head at the thought. What am I thinking? {{He}}'s a filthy, nasty little creature that
          probably hasn't had a bath for {{his}} entire life.` },
  ...
  onFinish: async choices => {
    ...
    if (ArrayUtility.contains(['normal','filthy'],choices.sex)) {
      Game.chainEvent(`found-fruits-and-nuts-sex-${(await Player.instance()).genderCode == 'female' ? 'F':'M'}`,{ style:choices.sex });
      SexEvent.add({
        eventType: 'event',
        character_id: choices.event.actorIDs.C,
        oralCount: 1,
      });
    }
  },
