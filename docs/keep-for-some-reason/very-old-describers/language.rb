module Language
  ...
  def all_both number
    raise "Number (#{number}) must be > 1" unless number > 1
    (number > 2) ? "all" : "both"
  end
  ...
  def number_to_english number, options = {}
    if number == 1 && options[:or].present?
      return options[:or]
    end
    ...
    case number
      ...
      else raise "This needs more numbers! [#{number}]"
    end
  end
