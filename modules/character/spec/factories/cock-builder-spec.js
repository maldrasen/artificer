describe('CockBuilder', function() {
  ...
  it("does nothing to character's without cocks", function(done) {
    MinionBuilder.buildMinion({ species:'lupin', gender:'female' }).then(character => {
  ...
  it("builds a cock of average length for that character's species (elf)", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'male', species:'elf' }).then(character => {
      character.getCock().then(cock => {
  ...
  it("builds a cock of average length for that character's species (centaur)", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'futa', species:'centaur' }).then(character => {
      character.getCock().then(cock => {
        expect(cock.convertedLength).to.be.within(10,32);
        expect(cock.convertedWidth).to.be.within(1.5,4.5);
        expect(cock.convertedScrotumWidth).to.be.within(3.5,12.5)
        done();
      });
    });
  });
  ...
  it("builds a cock of average length for that character's species (scaven)", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'male', species:'scaven' }).then(character => {
      character.getCock().then(cock => {
  ...
  it("sets a random knot width ratio if it's missing on a dog cock", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'futa', species:'lupin' }).then(character => {
      character.getCock().then(cock => {
  ...
  it("sets a random knot width ratio if it's missing on a dog cock", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'male', species:'caprien' }).then(character => {
      character.getCock().then(cock => {
  ...
  it('gives naga internal balls and two cocks', function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'futa', species:'naga' }).then(character => {
      character.getCock().then(cock => {
        expect(cock.count).to.equal(2);
