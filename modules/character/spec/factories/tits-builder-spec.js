describe('TitsBuilder', function() {
  ...
  it("doesn't give tits to Kobolds", function(done) {
    MinionBuilder.buildMinion({ firstName:'X', species:'kobold', gender:'female' }).then(character => {
      character.getTits().then(tits => {
  ...
  it('randomizes options otherwise', function(done) {
    MinionBuilder.buildMinion({ firstName:'X', gender:'futa', species:'sylph' }).then(character => {
      character.getTits().then(tits => {
