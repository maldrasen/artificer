...
//         Abuser.addHeadInjury(jada, { type:'cut', level:Random.between(1,2), count:Random.between(1,3) }).then(() => {
...
//
//         Abuser.addHeadInjury(jada, { type:'smash', level, count }).then(() => {
//           jada.getMouth().then(mouth => {
//             jada.getBody().then(body => {
...
//     SpecHelper.tenTimes(done, resolve => {
//       SpecHelper.buildRando().then(jada => {
//         Abuser.addBodyInjury(jada, { type:'pierce', level:3, count:Random.between(1,3) }).then(() => {
//           jada.getBody().then(body => {
