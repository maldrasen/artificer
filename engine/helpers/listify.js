global.listify = (function() {
  ...
  function oneWord(list) {
    ...
    const number = EnglishUtility.numberInEnglish(count)
  ...
  function twoWords(list) {
    ...
    const a_word_1 = EnglishUtility.a_an(word_1);
    const a_word_2 = EnglishUtility.a_an(word_2);
    const number_1 = EnglishUtility.numberInEnglish(count_1)
    const number_2 = EnglishUtility.numberInEnglish(count_2)
  ...
  function threeWords(list) {
    ...
    for (i=0; i<list.length; i++) {
      ...
      phrase += (i < list.length - 1 && list[i].count == 1) ?
        EnglishUtility.a_an(list[i].word):
        oneWord([list[i]]);
    }
